...
TRAIN_OPTIONS="--lr_decay_wait 2 --lr_decay_factor 0.6 --min_learning_rate 1e-4 --max_grad_norm 5 --num_steps 10  --learning_rate 1e-3 --debug --optim adam"
...
for RATE in $DROP_RATES; do
  ...
  rm_if_exist -r $EXP_DIR
  mkdir -p $EXP_DIR
  ipython -- train_joint_lm_dm_ind.py $MODEL_OPTIONS $TRAIN_OPTIONS --log_file_path $EXP_DIR"training.log" --output_dir $EXP_DIR --data_dir data/ptb-100/drop_$RATE/preprocess/ --dm_loss_weight 0.3 --def_dir data/common_defs_v1.2/preprocess/ --shared_emb_vocab $SHARED_EMB_VOCAB

  EXP_DIR="experiments/ptb-100/"$RATE"-reset-shared-dm/"
  rm_if_exist -r $EXP_DIR
  mkdir -p $EXP_DIR
  ipython -- train_joint_lm_dm_ind.py $MODEL_OPTIONS $TRAIN_OPTIONS --log_file_path $EXP_DIR"training.log" --output_dir $EXP_DIR --data_dir data/ptb-100/drop_$RATE/preprocess/ --dm_loss_weight 0.3 --def_dir data/common_defs_v1.2/preprocess/ --shared_emb_vocab $SHARED_EMB_VOCAB --shared_emb_lm_logit
done
